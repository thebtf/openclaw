...
const log = createSubsystemLogger("agent/google-payload");

function resolvePayloadLogConfig(env: NodeJS.ProcessEnv): PayloadLogConfig {
  const enabled = parseBooleanValue(env.OPENCLAW_GOOGLE_PAYLOAD_LOG) ?? false;
  const fileOverride = env.OPENCLAW_GOOGLE_PAYLOAD_LOG_FILE?.trim();
  const filePath = fileOverride
    ? resolveUserPath(fileOverride)
    : path.join(resolveStateDir(env), "logs", "google-payload.jsonl");
...
function isGoogleModel(model: Model<Api> | undefined | null): boolean {
  return (model as { api?: unknown })?.api === "google-generative-ai";
}
...
export type GooglePayloadLogger = {
  enabled: true;
  wrapStreamFn: (streamFn: StreamFn) => StreamFn;
  recordUsage: (messages: AgentMessage[], error?: unknown) => void;
};

export function createGooglePayloadLogger(params: {
  ...
  const wrapStreamFn: GooglePayloadLogger["wrapStreamFn"] = (streamFn) => {
  ...
  const recordUsage: GooglePayloadLogger["recordUsage"] = (messages, error) => {
  ...
  log.info("google payload logger enabled", { filePath: writer.filePath });

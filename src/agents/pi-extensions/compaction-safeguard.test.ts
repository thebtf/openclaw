...
describe("compaction-safeguard unresolved tool_call guard", () => {
  ...
  it("proceeds with compaction when tool_calls are all resolved", async () => {
    ...
    const result = (await compactionHandler(eventWithResolvedTools, mockContext)) as {
      cancel?: boolean;
    };

    // Should NOT cancel — all tool_calls are resolved
    expect(result).not.toEqual({ cancel: true });
    // getApiKey should have been called — handler proceeds past the guard
    expect(getApiKeyMock).toHaveBeenCalled();
  });
});
